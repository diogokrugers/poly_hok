...
def new_glist(list, shape, type) do
  flat = List.flatten(list)
  binary = for x <- flat, into: <<>>, do: glist_number_to_binary(x, type)
  {rows, cols} = shape
  ref = case type do
    {:f, 32} -> create_gpu_array_nx_nif(binary, rows, cols, to_charlist("float"))
    {:f, 64} -> create_gpu_array_nx_nif(binary, rows, cols, to_charlist("double"))
    {:s, 32} -> create_gpu_array_nx_nif(binary, rows, cols, to_charlist("int"))
    _ -> raise "Tipo não suportado: #{inspect(type)}"
  end
  {:nx, type, shape, [nil, nil], ref}
end

defp glist_number_to_binary(num, type) do
  case type do
    {:f, 32} -> <<num::float-32-little>>
    {:f, 64} -> <<num::float-64-little>>
    {:s, 32} -> <<num::signed-32-little>>
    _ -> raise "Tipo não suportado: #{inspect(type)}"
  end
end

def get_glist({:nx, type, shape, _names, ref}) do
  {l, c} = shape
  binary = case type do
    {:f, 32} -> get_gpu_array_nif(ref, l, c, to_charlist("float"))
    {:f, 64} -> get_gpu_array_nif(ref, l, c, to_charlist("double"))
    {:s, 32} -> get_gpu_array_nif(ref, l, c, to_charlist("int"))
    _ -> raise "Tipo não suportado: #{inspect(type)}"
  end
  glist_binary_to_list(binary, type)
end

defp glist_binary_to_list(binary, type) do
  case type do
    {:f, 32} -> for <<x::float-32-little <- binary>>, do: x
    {:f, 64} -> for <<x::float-64-little <- binary>>, do: x
    {:s, 32} -> for <<x::signed-32-little <- binary>>, do: x
    _ -> raise "Tipo não suportado: #{inspect(type)}"
  end
end
